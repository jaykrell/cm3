...
int
__cdecl
GetHostByName(const char* s, int* res)
// Get the IP4 address of a host, given its name.
{
    ...
    hostent* const h = gethostbyname(s);
    if (h)
    {
        *res = GetAddress(h);
        return 0; // no error
...
int
__cdecl
GetCanonicalByName(const char* s, TEXT* text)
{
    hostent* h;
    int err = 0;
    ...
    const char* port)
{
    ...
    while (addressInfo)
    {
        ...
        if (family == AF_INET)
        {
            ...
            IPInternal__NewEndpoint4(ep, sa->sin_port, &sa->sin_addr);
            break;
        }
        else if (family == AF_INET6)
        {
            sockaddr_in6* sa = (sockaddr_in6*)addressInfo->ai_addr;
            IPInternal__NewEndpoint6(ep, sa->sin6_port, &sa->sin6_addr);
            break;
        }
